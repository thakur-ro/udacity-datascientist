...
def clean_text(line):
    """
    Input:
        line: (string) product description
    output:
        line: (string) cleaned product description
    """
...
def product_description_gen(df):
    """
    Input:
    df: Pandas.DataFrame (containing product features)
    Output:
    df: Pandas.DataFrame (generate and tokenize product description and title)
    """
...
def clean_df(ratings, metadata, num_items, min_history):
    ...
    num_items: (int) action space
    min_history: (int) state space
    Output:
