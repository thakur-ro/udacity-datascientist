...
def create_history(df):
    """
    Input: DataFrame with user items interaction history
    Output: historic users list
    """
...
def create_sample_sequence(user_history, min_num_states, min_num_actions, states_ratio, sample_size):
    """
    Input:
    user_history: (list) of historic items interaction for a user
    min_num_states: (int) minimum no. of states to create
    min_num_actions:  (int) minumum no of actions
    states_ratio: (float) percentage of items taken for state
    sample_size: (int) sample size

    Output:
     (list) states and actions for user
    """
    n = len(user_history)
